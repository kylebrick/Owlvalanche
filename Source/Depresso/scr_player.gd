...
#@onready var obj_wings:				AnimatedSprite2D	= $Wings;
@onready var obj_wings:				PackedScene			= preload("res://obj_wings.tscn");
...
var inst_wings:					AnimatedSprite2D;
...
func _physics_process(delta: float) -> void:
	...
	if(is_on_floor()):
		...
		if(!landed):
			...
			if(is_instance_valid(inst_wings)): inst_wings.queue_free();
			crouch_reset();
		...
		if(can_coyote_jump):
			...
			if(Input.is_action_just_pressed("jump")) && (can_double_jump):
				...
				#Wings
				if(!is_instance_valid(inst_wings)): 
					inst_wings = obj_wings.instantiate();
					inst_wings.position = Vector2(0,-12);
					inst_wings.flip_h = spr.flip_h;
					add_child(inst_wings);
...
func double_jump():
	...
	#var wings = obj_wings.instantiate();
func animate():
	...
	if(velocity.x < 0):
		...
		if(is_instance_valid(inst_wings)):
			inst_wings.flip_h = true;
	elif(velocity.x > 0):
		spr.flip_h = false;
		fx_footsteps.position.x = -fx_footsteps_offset;
		if(is_instance_valid(inst_wings)):
			inst_wings.flip_h = false;
